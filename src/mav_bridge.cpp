...
MavBridge::MavBridge() {}
MavBridge::~MavBridge() {}
void MavBridge::init(HardwareSerial *serial, uint32_t baudrate, uint8_t system_id,
  uint8_t component_id, uint8_t steering_channel, uint8_t throttle_channel, uint8_t message_rate) {
    m_serial = serial;
    m_baudrate = baudrate;
    ...
    for (uint16_t i = 0; i < len; i++) {
        ...
        if (mavlink_parse_char(MAVLINK_COMM_0, c, &msg, &status)) {
            if (msg.msgid == MAVLINK_MSG_ID_SCALED_IMU) {
                ...
                m_arm_state = hb.base_mode & MAV_MODE_FLAG_SAFETY_ARMED;  // Check if FC is armed
                if (m_arm_state != m_requested_arm_state) {
                    set_arm(m_requested_arm_state);
                }
            }
        }
    }
...
float MavBridge::get_gyro_data() { return m_gyro_data; }

float MavBridge::get_steering_pct() { return m_steering_pct; }
float MavBridge::get_throttle_pct() { return m_throttle_pct; }
bool MavBridge::get_arm_state() { return m_arm_state; }
...
void MavBridge::set_message_rate(uint32_t msg_id, uint16_t message_rate_hz) {
    ...
    mavlink_msg_command_long_pack(m_system_id,  // Your system ID
...
void MavBridge::toggle_arm() {
    m_requested_arm_state = !m_requested_arm_state;
    set_arm(m_requested_arm_state);
}

void MavBridge::set_arm(int arm) {
    mavlink_message_t msg;
    uint8_t buf[MAVLINK_MAX_PACKET_LEN];
